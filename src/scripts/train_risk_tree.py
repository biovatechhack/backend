...
Feature groups
--------------
Profile  : age, gender (0=M/1=F), bmi
Context  : hba1c, has_hypertension, has_heart_disease, medication_count
Vitals   : glucose, hr, spo2, steps, sleep_hours
Symptoms : confusion, tremors, thirst   (mock Gemini extraction)

Usage
-----
    python -m src.scripts.train_risk_tree
"""
...
def _generate_dataset(n_samples: int = 2000) -> pd.DataFrame:
    ...
    for _ in range(n_samples):
        ...
        # Chronic context — comorbidities correlated with age and BMI
        ...
        medication_count  = int(np.clip(
            rng.integers(0, 3) + has_hypertension + has_heart_disease, 0, 6
        ))

        # Lifestyle / sensor inputs
...
def train() -> None:
    ...
    dist = df["risk_state"].value_counts().sort_index()
    for idx, label in enumerate(RISK_LABELS):
        print(f"  {label:<10} {dist.get(idx, 0)} samples")
    print()
